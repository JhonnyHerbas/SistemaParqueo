...
function visualizar_sitio_compartido_id($id)
{
    $conn = get_connection();
    $query = 'CALL DB_SP_COMPARTIDO_VISTA_ID("' . $id . '")';
    if ($result = $conn->query($query)) {
        return $result;
    } else {
        return null;
...
function rechazar_sitio_compartido($ID)
{
    $conn = get_connection();
    $query = 'CALL DB_SP_COMPARTIDO_RECHAZO("' . $ID . '")';
    if ($result = $conn->query($query)) {
        return $result;
    } else {
        return null;
...
function aceptar_sitio_compartido($ID)
{
    $conn = get_connection();
    $query = 'CALL DB_SP_COMPARTIDO_ACEPTAR("' . $ID . '")';
    if ($result = $conn->query($query)) {
        return $result;
    } else {
        return null;
